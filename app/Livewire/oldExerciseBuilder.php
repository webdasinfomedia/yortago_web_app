   ...
   private function loadWeeks()
{
                ...
                'days' => $week->days->sortBy('day_number')->map(function ($day, $dayIndex) {
                    ...
                    return [
                        ...
                        'exercises' => $day->exerciseItems->map(function ($ex, $exIndex) {
                            ...
                            // Get linked alternate exercises
                            $alternates = AlternateExerciseList::where('new_exercise_week_day_item_id', $ex->id)
                                ->get()
                                ->map(function($alt) {
                                    return [
                                        'id' => $alt->id,
                            ...
                            return [
                                ...
                                'notes' => $ex->notes,
                                'alternates' => $alternates, // Add alternates here
                            ];
                        })->toArray()
                    ];
                })->values()->toArray()
    ...
    public function selectDay($dayId)
    {
        ...
        // Keep active week instead of resetting
        $day = collect($this->weeks)->flatMap(fn($w) => $w['days'])->firstWhere('id', $dayId);
        if ($day) {
            $week = collect($this->weeks)->firstWhere('id', $day['week_id'] ?? null);
            if ($week) {
    ...
    public function toggleWeek($weekId)
    {
        if ($this->activeWeekAccordion === $weekId) {
            ...
            // Auto-select first day in that week
            $week = collect($this->weeks)->firstWhere('id', $weekId);
            if ($week && !empty($week['days'])) {
                $this->selectedDayId = $week['days'][0]['id'];
            }
    ...
    public function updateExercise($exerciseId, $field, $value)
{
    ...
    // Refresh the specific exercise data
    $this->loadWeeks();
}
    public function saveAllExercises()
    {
        ...
        if ($savedCount === $totalCount && $totalCount > 0) {
            ...
            $this->dispatch('show-error', message: 'No exercises to save for this week.');
        }
    }
   ...
   public function addAlternateExercise($mainExerciseId)
{
    ...
    // Get available alternate exercises (either null or linked to other exercises)
    $availableAlternates = AlternateExerciseList::where('exercise_list_id', $mainExercise->exercise_list_id)
        ->where(function($query) use ($mainExerciseId) {
            $query->whereNull('new_exercise_week_day_item_id')
                  ->orWhere('new_exercise_week_day_item_id', '!=', $mainExerciseId);
        })
        ->get();
    ...
    // Link it to the main exercise with empty values for user to fill
    $alternate->update([
    ...
    $this->loadWeeks();
    $this->dispatch('show-success', message: 'Alternate exercise added! Please fill in the details and click Save All Exercises.');
}
