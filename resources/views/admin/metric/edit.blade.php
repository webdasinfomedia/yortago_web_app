...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-lg-8 col-md-4">
            <div class="card">
                ...
                <div class="card-body">
                    <form id="metricForm" method="post" action="{{ route('admin.metrics.update') }}">
                        @csrf
                        <input type="hidden" name="id" value="{{ $metric->id }}">

                        <div class="form-group">
                            <label>Name</label>
                            <input type="text" class="form-control" name="name" id="name" placeholder="Name" value="{{ $metric->name }}" >
                            <small id="nameError" class="text-danger"></small>
                        </div>

                        <div id="parameters-container">
    @if(isset($metric->parameters) && $metric->parameters->isNotEmpty())
        @foreach($metric->parameters as $item)
            <div class="parameter-row">
                <label>Parameter</label>
                <input type="text" name="parameter[]" value="{{ old('parameter.' . $loop->index, $item->name) }}" class="form-control parameter-input" placeholder="Parameter" required>
                
                <label>Color</label>
                <input type="color" name="color[]" value="{{ old('color.' . $loop->index, $item->color_code) }}" class="form-control color-input">
        ...
        @endforeach
    @else
        <!-- Display a default row if there are no parameters -->
        <div class="parameter-row">
            <label>Parameter</label>
    ...
    @endif
                    ...
                    <div id="successMessage" class="alert alert-success d-none">Metric updated successfully!</div>
                </div>
            </div>
        </div>
    </div>
...
@section('script')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Validate that only numbers are entered in parameter fields
            // parametersContainer.addEventListener('input', function(e) {
            ...
            const validateForm = () => {
                ...
                parameterRows.forEach(row => {
                    ...
                    // Show red border if field is empty
                    if (parameterInput.value.trim() === '') {
                        parameterInput.classList.add('is-invalid');
                        valid = false;
                    } else {
                        parameterInput.classList.remove('is-invalid');
                    }
                });
            ...
            form.addEventListener('submit', function (e) {
                ...
                fetch("{{ route('admin.metrics.update') }}", {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data && data.status === 200) {
                        ...
                        setTimeout(() => {
                            window.location.href = "{{ route('admin.metrics.list') }}";
                        }, 1000);
                    }
                })
